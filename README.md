# TIL

> 배운 것을 기록하자

### Algorithm

- [안정 정렬과 불안정 정렬](Algorithm/안정-정렬과-불안정-정렬.md)
- [다이나믹 프로그래밍](Algorithm/다이나믹-프로그래밍.md)
- [Greedy 알고리즘](Algorithm/Greedy-알고리즘.md)

### CleanCode

- [9장 단위 테스트](CleanCode/9장-단위-테스트.md)
- [6장 객체와 자료구조](CleanCode/6장-객체와-자료구조.md)
- [3장 함수](CleanCode/3장-함수.md)
- [7장 에러 처리](CleanCode/7장-에러-처리.md)
- [5장 형식 맞추기](CleanCode/5장-형식-맞추기.md)
- [4장 주석](CleanCode/4장-주석.md)
- [10장 클래스](CleanCode/10장-클래스.md)
- [2장 의미있는 이름](CleanCode/2장-의미있는-이름.md)

### Etc

- [32bit와 64bit 어플리케이션](Etc/32bit와-64bit-어플리케이션.md)
- [프레임워크와 라이브러리](Etc/프레임워크와-라이브러리.md)

### Git

- [revert로 원격 레포 커밋 되돌리기](Git/revert로-원격-레포-커밋-되돌리기.md)
- [특정 커밋 수정하기](Git/특정-커밋-수정하기.md)
- [Git 브랜치 사용하기](Git/Git-브랜치-사용하기.md)
- [작업 여기저기로 옮기기 with 체리픽,리베이스](Git/작업-여기저기로-옮기기-with-체리픽,리베이스.md)
- [amend로 기존 커밋 수정하기](Git/amend로-기존-커밋-수정하기.md)
- [Git 기본 명령어](Git/Git-기본-명령어.md)
- [커밋트리에서 이동하기](Git/커밋트리에서-이동하기.md)
- [커밋메시지 convention과 커밋 template 만들기](Git/커밋메시지-convention과-커밋-template-만들기.md)
- [stash로 임시 보관하기](Git/stash로-임시-보관하기.md)

### GraphQL

- [GraphQL과 REST API](GraphQL/GraphQL과-REST-API.md)

### Infra

- [현재 위치 배포와 블루 그린 배포](Infra/현재-위치-배포와-블루-그린-배포.md)

### Network

- [TCP와 UDP](Network/TCP와-UDP.md)
- [3 way handshake & 4 way handshake](Network/3-way-handshake-&-4-way-handshake.md)

### Node.js

- [Express cookie 사용하기](Node.js/Express-cookie-사용하기.md)
- [passport local 로그인 구현하기](Node.js/passport-local-로그인-구현하기.md)
- [passport facebook 타사인증 구현하기](Node.js/passport-facebook-타사인증-구현하기.md)
- [Express session 사용하기](Node.js/Express-session-사용하기.md)

### OS

- [커널 레벨 스레드와 유저 레벨 스레드](OS/커널-레벨-스레드와-유저-레벨-스레드.md)
- [Inter Process Communication](OS/Inter-Process-Communication.md)
- [페이지 교체 알고리즘](OS/페이지-교체-알고리즘.md)
- [CPU 스케쥴링 알고리즘](OS/CPU-스케쥴링-알고리즘.md)
- [스레싱](OS/스레싱.md)
- [메모리 할당 기법](OS/메모리-할당-기법.md)
- [RAID](OS/RAID.md)
- [프로세스와 스레드](OS/프로세스와-스레드.md)
- [임계구역 문제](OS/임계구역-문제.md)
- [프로세스](OS/프로세스.md)
- [교착상태](OS/교착상태.md)

### Prisma

- [Prisma의 checkExistence](Prisma/Prisma의-checkExistence.md)
- [Prisma 시작하기](Prisma/Prisma-시작하기.md)
- [Prisma의 Datamodel](Prisma/Prisma의-Datamodel.md)

### Web

- [Rest API란 무엇인가](Web/Rest-API란-무엇인가.md)
- [HTTP 응답 상태코드](Web/HTTP-응답-상태코드.md)
- [URI와 URL와 URN](Web/URI와-URL와-URN.md)
- [Rest API 설계 가이드](Web/Rest-API-설계-가이드.md)

